...
df <- read_sas("Facility Inventory/tanzania 2014-15/TZFC71SDSR/TZFC71FLSR.SAS7BDAT")
...
shape <- shapefile("Facility Inventory/tanzania 2014-15/geo/tza_admbnda_adm0_20181019.shp")
plot(shape, main="Shape for Clipping")
friction <- malariaAtlas::getRaster(
  surface = "A global friction surface enumerating land-based travel speed for a nominal year 2015",
  #shp = shape,
  extent = matrix(c("29.5", "-12","40.5", "-0.5"), nrow = 2, ncol = 2, dimnames = list(c("x", "y"), c("min", "max"))))
...
point.locations <- read.csv(file = "Facility Inventory/tanzania 2014-15/geo/moh_location.csv")
...
df_clusterlatlong <- read.dbf("DHS/Tanzania/TZGE7AFL/TZGE7AFL.dbf") %>%
  dplyr::select(cluster_id = DHSCLUST, latitude = LATNUM, longitude = LONGNUM) %>%
  filter(latitude != 0 & longitude != 0)

#Import DHS data
df_hh <- read_sas("DHS/Tanzania/TZHR7BSD/TZHR7BFL.SAS7BDAT")
...
tanzania[, c("xvar", "yvar")] <- latlong2grid( 
  tanzania[, c("longitude", "latitude")]
)
...
tanzania <- dist.merge(tanzania, df_cluster, 'xvar', 'yvar', 'xvar', 'yvar') 

ggplot(tanzania, aes(x=min.dist)) + geom_histogram()
ggplot(tanzania, aes(x=travel_time, y=hh.wealthindex.mean)) + geom_point() + geom_smooth()
